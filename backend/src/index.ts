...
interface SubCategory extends DictionaryEntry {
  category_id: number;
}
...
const subcategories: SubCategory[] = [
  { id: 1, category_id: 1, name: 'Приём терапевта' },
  { id: 2, category_id: 1, name: 'Приём специалиста' },
  { id: 3, category_id: 2, name: 'Сдача анализов' },
  { id: 4, category_id: 5, name: 'Жалоба на сервис' }
];
...
app.get('/calls', (req, res) => {
  ...
  if (sub_category_id) {
    const id = Number(sub_category_id);
    result = result.filter((c) => c.sub_category_id === id);
  }
  if (tag_id) {
    const id = Number(tag_id);
    result = result.filter((c) => c.tag_ids?.includes(id));
  }
  if (from) {
...
app.put('/calls/:id', (req, res) => {
  ...
  const { purpose_id, category_id, notes, sub_category_id, tag_ids } =
    req.body as Partial<Call>;
  ...
  call.sub_category_id = sub_category_id;
  call.tag_ids = tag_ids || [];
  call.notes = notes;
  res.json(call);
});
...
app.get('/export/calls.csv', (_req, res) => {
  res.type('text/csv');
  const lines = [
    'id,src_number,dst_number,status,category_id,sub_category_id,tag_ids'
  ];
  for (const c of calls) {
    lines.push(
      `${c.id},${c.src_number},${c.dst_number},${c.status},${c.category_id ?? ''},${c.sub_category_id ?? ''},${(c.tag_ids || []).join('|')}`
    );
  }
  res.send(lines.join('\n'));
});
